...
class bahdanau_attention(nn.Module):
    ...
    def forward(self, h, x):
        gamma_encoder = self.linear_encoder(self.context)  # [batch, len, size]
        gamma_decoder = self.linear_decoder(h).unsqueeze(1)  # [batch, 1, size]
        weights = self.linear_v(self.tanh(gamma_encoder + gamma_decoder)).squeeze(
            2
        )  # [batch, len]
        weights = self.softmax(weights)  # [batch, len]
        c_t = torch.bmm(weights.unsqueeze(1), self.context).squeeze(1)  # [batch, size]
...
class Multihead_Attention(nn.Module):
    ...
    def __init__(self, model_dim, head_count=8, dropout=0.1):
        ...
        self.linear_keys = nn.Linear(model_dim, head_count * self.head_dim)
        self.linear_values = nn.Linear(model_dim, head_count * self.head_dim)
        self.linear_query = nn.Linear(model_dim, head_count * self.head_dim)
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(
        ...
        def shape(x):
            """projection"""
            return x.view(batch_size, -1, head_count, head_dim).transpose(
                1, 2
            )  # [batch, head, len, head_dim]
            ...
            if type == "self":
                ...
                if layer_cache is not None:
                    ...
                    if layer_cache["self_keys_" + device] is not None:
                        key = torch.cat(
                            (layer_cache["self_keys_" + device], key), dim=2
                        )  # [batch, head, k_len, head_dim]
                    if layer_cache["self_values_" + device] is not None:
                        value = torch.cat(
                            (layer_cache["self_values_" + device], value), dim=2
                        )  # [batch, head, v_len, head_dim]
            ...
            # decoder context attention in the inference stage
                ...
                if layer_cache is not None:
                    ...
                    if layer_cache["memory_keys_" + device] is None:
                        ...
                        key, value = (
                            layer_cache["memory_keys_" + device],
        ...
        if tau is not None:
            tau = tau.view(batch_size, -1, head_count, 1).transpose(
                1, 2
            )  # [batch, head, len, 1]
            tau_len = tau.size(2)
