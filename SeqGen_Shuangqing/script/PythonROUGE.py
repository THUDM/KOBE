...
def PythonROUGE(guess_summ_list, ref_summ_list, ngram_order=2, byte=0):
    """ Wrapper function to use ROUGE from Python easily. """
    ...
    options = '-w 1.2 -a -m -n ' + str(ngram_order)
    if byte > 0:
    ...
    exec_command = 'perl ' + ROUGE_path + ' -e ' + data_path + \
        ' ' + options + ' ' + xml_path + ' > ' + ROUGE_output_path
...
def create_xml(xml_file, guess_summ_file, ref_summ_list):
    ...
    for ref_summ_index, ref_summ_file in enumerate(ref_summ_list):
        ...
        xml_file.write(
            '<M ID="' + letter_list[ref_summ_index] + '">' + ref_summ_basename + '</M>\n')
    xml_file.write('</MODELS>\n')
...
if __name__ == '__main__':
    guess_summary_list = [
        'Example/Guess_Summ_1.txt', 'Example/Guess_Summ_2.txt']
    ref_summ_list = [['Example/Ref_Summ_1_1.txt', 'Example/Ref_Summ_1_2.txt'],
                     ['Example/Ref_Summ_2_1.txt', 'Example/Ref_Summ_2_2.txt', 'Example/Ref_Summ_2_3.txt']]
    recall_list, precision_list, F_measure_list = PythonROUGE(
        guess_summary_list, ref_summ_list)
