...
if config.tensorboard:
    ...
    writer = SummaryWriter(config.tensorboard_log_dir, comment="a")
    ...
    for src, tgt, src_len, tgt_len, original_src, original_tgt in trainloader:
        ...
        if params['updates'] % config.report_interval == 0:
            params['log']("epoch: %3d, loss: %6.3f, time: %6.3f, updates: %8d, accuracy: %2.2f\n"
                          % (epoch, params['report_loss'] / config.report_interval, time.time()-params['report_time'],
                             params['updates'], params['report_correct'] * 100.0 / params['report_total']))

            if config.tensorboard:
                writer.add_scalar("train" + "/loss",
                                  params['report_loss'], params['updates'])
                writer.add_scalar("train" + "/lr", optim.lr, params['updates'])
                writer.add_scalar(
                    "train" + "/accuracy", params['report_correct'] * 100.0 / params['report_total'], params['updates'])
