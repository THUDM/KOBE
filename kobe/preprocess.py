...
parser.add_argument("--load_data", required=True, help="input file for the data")
...
parser.add_argument(
    "--src_vocab_size", type=int, default=50000, help="Size of the source vocabulary"
)
parser.add_argument(
    "--tgt_vocab_size", type=int, default=50000, help="Size of the target vocabulary"
)
parser.add_argument(
    "--src_filter", type=int, default=0, help="Maximum source sequence length"
)
parser.add_argument(
    "--tgt_filter", type=int, default=0, help="Maximum target sequence length"
)
parser.add_argument(
    "--src_trun", type=int, default=0, help="Truncate source sequence length"
)
parser.add_argument(
    "--tgt_trun", type=int, default=0, help="Truncate target sequence length"
)
...
parser.add_argument(
    "--src_suf", default="src", help="the suffix of the source filename"
)
parser.add_argument(
    "--tgt_suf", default="tgt", help="the suffix of the target filename"
)
parser.add_argument("--lower", action="store_true", help="lower the case")
parser.add_argument(
    "--share",
    action="store_true",
    help="share the vocabulary between source and target",
)
parser.add_argument("--freq", type=int, default=0, help="remove words less frequent")
...
def makeVocabulary(filename, trun_length, filter_length, char, vocab, size, freq=0):
    ...
    print(
        "%s: length limit = %d, truncate length = %d"
...
def makeData(
    srcFile, tgtFile, srcDicts, tgtDicts, save_srcFile, save_tgtFile, trun=True
):
    sizes = 0
    ...
    print(
        "Prepared %d sentences (%d and %d ignored due to length == 0 or > )"
...
def main():
    ...
    train_src, train_tgt = (
        opt.load_data + "train." + opt.src_suf,
        opt.load_data + "train." + opt.tgt_suf,
    )
    valid_src, valid_tgt = (
        opt.load_data + "valid." + opt.src_suf,
        opt.load_data + "valid." + opt.tgt_suf,
    )
    test_src, test_tgt = (
        opt.load_data + "test." + opt.src_suf,
        opt.load_data + "test." + opt.tgt_suf,
    )

    save_train_src, save_train_tgt = (
        opt.save_data + "train.src",
        opt.save_data + "train.tgt",
    )
    save_valid_src, save_valid_tgt = (
        opt.save_data + "valid.src",
        opt.save_data + "valid.tgt",
    )
    save_test_src, save_test_tgt = (
        opt.save_data + "test.src",
        opt.save_data + "test.tgt",
    )
    ...
    train = makeData(
        train_src, train_tgt, dicts["src"], dicts["tgt"], save_train_src, save_train_tgt
    )
